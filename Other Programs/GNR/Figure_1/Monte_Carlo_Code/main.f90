PROGRAM MAIN
  ...
  INTEGER :: i,j,s,t
  REAL(8) :: theta(1),dist,point(3),it,etat,epst,prod,kt1,aux,lo,hi,iti,mti,yti,lrptm1,wtm1,lrptv(N_time),kti,kt1i
  REAL(8), POINTER :: fx_ch0(:,:),fx_ch_temp(:)
  INTEGER :: num1,num2,num3,num4
  INTEGER, POINTER :: mylo(:),myhi(:)
  INTEGER :: ierr,myid,numprocs,jj
  ...
  limits(:,1)=(/1.0d0,-2.0d0,0.5d0/)
  limits(:,2)=(/6000.0d0,2.0d0,6.0d0/)
  DO j=1,N_d
     CALL MLINEAR_INIT(vt1(j),(/20,20,20/),limits,(/1,0,0/))
     CALL MLINEAR_INIT(i_pol(j),(/20,20,20/),limits,(/1,0,0/))
  END DO
  ALLOCATE(fx_ch(N_d,vt1(1)%lt),fx_ch0(N_d,vt1(1)%lt),i_ch(N_d,vt1(1)%lt))
  ALLOCATE(fx_ch_temp(vt1(1)%lt))
  
  CALL GAUSS_HERMITE(ab,we)
  AB_P=AB*sqrt(2.0d0*var_pr)
  ...
  DO h=1,N_d
      ...
      DO
         ...
         DO j=mylo(myid),myhi(myid)
            ...
            lrpt=vt1(h)%v(2)%x(vt1(h)%i(j)%i(2))
            wt=vt1(h)%v(3)%x(vt1(h)%i(j)%i(3))
            mt=(LOG(am)+ak*LOG(kt)+LOG(wt)-lrpt) / (1.0d0-am)
            mt=EXP(mt)
            theta=20.0d0
...
!               CALL BFGS(theta,1.0d-10,1.0d-10,OBJECTIVE,myid,.FALSE.)
   ...
   IF (myid==0) THEN         
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Change the name of this file to correspond to the level of time series variation !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! specified in globvar.f90 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      OPEN(523,file="base_v_data.out")
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      DO jj=1,500
         ! Ok so now I have the solution to the Bellman Problem, let's create firms
         lrptm1=Sample_Normal(0.0d0,0.5d0)
         DO t=1,N_time
            lrptv(t)=per_pr*lrptm1+Sample_Normal(0.0d0,var_pr)
            lrptm1=lrptv(t)
         END DO
         DO i=1,N_firms
            ...
            mtm1=Sample_Uniform(11.0d0,400.0d0)
            DO t=1,N_time
               kt=kt1
               kti=kt1i
               ...
               hi=limits(3,2)
               IF (wt>hi) wt=hi
               mt=(LOG(am)+ak*LOG(kt)+LOG(wt)-lrptv(t)) / (1.0d0-am)
               mt=EXP(mt)
               mti=mt
               point=(/kti,lrptv(t),wt/)
               ...
               yti=(kti**ak)*(mti**am)*wt*exp(epst)
               ! Now prepare for next period
               kt1=(1.0d0-d(h))*kt + it
               kt1i=(1.0d0-d(h))*kti + iti
               mtm1=mt
               wtm1=wt
               s=t-150
               IF (t>150) WRITE(523,'(4I10,12F16.8)') i,t,h,jj,kti,mti,yti,lrptv(t)           
            END DO
         END DO
      END DO
   END IF

PROGRAM MAIN
  ...
  INTEGER :: i,j,s,t
  REAL(8) :: theta(1),dist,point(2),it,etat,epst,wtm1,prod,kt1,aux,lo,hi,iti,mti,yti,mt_0
  ...
  DO h=1,N_d
      ...
      DO
         ...
         DO j=mylo(myid),myhi(myid)
            ...
            DO 
               lmt=LOG(mt_0)
               arg=(ak*(kt**sig))+(am*(mt_0**sig))
               arg=arg**((ret/sig)-1.0d0)
               arg=arg*(mt_0**sig)
               mt=P*ret*am*arg*wt/Pm
               IF (ABS(mt-mt_0)<ABS(mt_0)*1.0d-10) EXIT
               mt_0=mt
            END DO
            IF (mt<=0.0d0) mt=0.5d0
...
!            END IF
         ...
         IF (i<20) THEN
            DO s=1,100
               DO j=1,vt1(h)%lt
                  ...
                  !  mt=EXP(arg)*wt*(am+amm*2.0d0*lmt+akm*lkt)
                  !  IF (ABS(mt-mt_0)<ABS(mt_0)*1.0d-10) EXIT
                  !  mt_0=mt
                  !END DO
                  !IF (mt<=0.0d0) mt=0.5d0
         ...
         IF (ANY(abs(fx_ch(h,:)-fx_ch0(h,:))>0.0001d0* SQRT(SUM(fx_ch(h,:)*fx_ch(h,:))) )) THEN
            j=1
         ELSE
            EXIT
         END IF
   ...
   IF (myid==0) THEN
      ...
      DO i=1,N_firms
         ...
         DO t=1,N_time
            ...
            theta=20.0d0
            CALL NELDER_MEADE(theta,1.0d-10,OBJECTIVE,myid,1000000)
            CALL NELDER_MEADE(theta,1.0d-10,OBJECTIVE,myid,1000000)
            CALL NELDER_MEADE(theta,1.0d-10,OBJECTIVE,myid,1000000)
            CALL NELDER_MEADE(theta,1.0d-10,OBJECTIVE,myid,1000000)
            CALL NELDER_MEADE(theta,1.0d-10,OBJECTIVE,myid,1000000)
            ...
            lkt=LOG(kt)
            lmt=LOG(mt)
            arg=(ak*(kt**sig))+(am*(mt**sig))
            arg=arg**(ret/sig)
            prod=arg*wt*exp(epst)
            lmt=LOG(mti)
            arg=(ak*(kt**sig))+(am*(mti**sig))
            arg=arg**(ret/sig)
            yti=arg*wt*exp(epst)
